...
if(($useraccount == "" || $useraccount == null) || ($password == "" || $password == null) || ($passwordoublechk == "" || $passwordoublechk == null)){
    ...
    if($password != $passwordoublechk){
        ...
        //查詢帳號是否已經存在(驗到帳號有重複，就要去更改了)
        include("connect.php");
        $sql_check = "SELECT * FROM `member` WHERE `useraccount` = ?";
        $stmt_check = mysqli_prepare($db, $sql_check);
        mysqli_stmt_bind_param($stmt_check, "s", $useraccount);
        ...
        if(mysqli_num_rows($result_check) > 0){
            echo '<script>alert("這個帳號已經有人註冊了，請重新輸入！")</script>';
            echo '<meta http-equiv="refresh" content="0; url=./regist.php">';
        }
        else{
            //如果帳號沒人註冊過，就執行INSERT INTO語句
            $hash = hash("sha256", $password); // 將密碼做雜湊處理
            $sql_insert = "INSERT INTO `member`(`useraccount`, `password`) VALUES(?, ?)";
            $stmt_insert = mysqli_prepare($db, $sql_insert);
            mysqli_stmt_bind_param($stmt_insert, "ss", $useraccount, $hash);
