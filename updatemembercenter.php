...
if($password == "" || $passwordoublechk == "" || $nickname == ""){
    ...
    if($password != $passwordoublechk){
        ...
        // 看使用者輸入的密碼是否與舊的一樣 ， 暱稱欄位是否沒填
        if(mysqli_num_rows($result_check) != 0 || ($nickname == "" && isset($nickname)) ){
            echo '<script>alert("新密碼不能與舊密碼相同，請重新輸入！")</script>';
            echo '<meta http-equiv="refresh" content="0; url=./membercenter.php">';
        }
        else{
            //如果密碼沒有相同，就執行INSERT INTO語句，更新會員資料
            $sql = "UPDATE `member` SET `password` = ? AND                      `nickname` = ? WHERE `useraccount` = ?";
            $stmt = mysqli_prepare($db, $sql);            
            mysqli_stmt_bind_param($stmt, "sss", $hash, $nickname, $_SESSION["useraccount"]);
